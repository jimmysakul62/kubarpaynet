...
import { generateIDPelanggan } from './utils.js';
...
export function renderDataPelangganTable(filterLokasi = 'OPERATOR') {
    ...
    container.innerHTML = `
        <h2 class="page-title">DATA PELANGGAN ${currentKampungPelanggan !== 'OPERATOR' ? '- ' + currentKampungPelanggan : ''}</h2>
...
export function renderInputDataForm(namaKampungDefault = '', dataEdit = null) {
    ...
    document.getElementById('pelangganForm').addEventListener('submit', function(e) {
        ...
        showSuccessMessage(isEdit ? "Data Berhasil Diperbarui!" : "Data Pelanggan Baru Berhasil Disimpan!");
    });
}
...
window.confirmDelete = function(id) {
    if(confirm(`YAKIN? Menghapus pelanggan ID ${id} juga akan menghapus SEMUA data riwayat pembayarannya!`)) {
        ...
        if(indexPelanggan !== -1) {
            ...
            alert(`Selesai! Data ${namaDihapus} beserta riwayat pembayarannya telah dibersihkan.`);
            loadPelangganData(); 
        } else {
...
window.editPelanggan = function(id) {
    ...
    if(dataEdit) {
        ...
        alert('Error: Data pelanggan tidak ditemukan untuk diedit.');
    }
};